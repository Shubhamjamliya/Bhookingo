...
function ReviewModal({ booking, onClose, onSubmit }) {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <div className="bg-surface dark:bg-slate-900 rounded-3xl w-full max-w-md overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200 border dark:border-slate-800">
                <div className="p-6 border-b border-border dark:border-slate-800 flex items-center justify-between">
                    <h3 className="text-xl font-bold text-text-primary dark:text-white">Review your experience</h3>
                    <button onClick={onClose} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full transition-colors">
                        <X className="w-5 h-5 text-text-secondary dark:text-text-secondary" />
                    </button>
                </div>

                <div className="p-6 space-y-6">
                    <div className="flex flex-col items-center">
                        <p className="text-sm font-medium text-text-secondary dark:text-text-secondary mb-3">How was your visit to {booking.restaurant?.name}?</p>
                    ...
                    <div className="space-y-2">
                        ...
                        <textarea
                            ...
                            className="w-full h-32 p-4 bg-slate-50 dark:bg-slate-800 border-none rounded-2xl focus:ring-2 focus:ring-red-500 transition-all text-sm resize-none dark:text-white dark:placeholder:text-text-secondary"
                        />
                    </div>
                    ...
                    <Button
                        ...
                        disabled={submitting}
                        className="w-full bg-red-500 hover:bg-primary text-white font-bold h-12 rounded-2xl shadow-lg shadow-red-200"
                    >
...
export default function MyBookings() {
    ...
    const getStatusBadgeClass = (status) => {
        ...
        if (key === "checked-in") return "bg-background text-[var(--primary)]"
    ...
    return (
        <AnimatedPage className="bg-slate-50 dark:bg-[#0a0a0a] min-h-screen pb-10 transition-colors">
            <div className="bg-surface dark:bg-[#0a0a0a] p-4 flex items-center shadow-sm sticky top-0 z-10 border-b dark:border-gray-800">
            ...
            <div className="p-4 space-y-4">
                {bookings.length > 0 ? (
                    bookings.map((booking) => (
                        <div key={booking._id} className="bg-surface dark:bg-[#1a1a1a] rounded-2xl p-4 shadow-sm border border-border dark:border-gray-800 flex items-start gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex justify-between items-start">
                                    <h3 className="font-bold text-text-primary dark:text-white truncate">{booking.restaurant?.name}</h3>
                                ...
                                <div className="flex items-center gap-4 mt-3">
                                    <div className="flex items-center gap-1 text-[11px] font-bold text-text-secondary dark:text-gray-300 bg-slate-100 dark:bg-gray-800 px-2 py-0.5 rounded-lg">
                                ...
                                {booking.status === 'completed' && (
                                    <button
                                        ...
                                        className="mt-3 w-full py-2 bg-primary-light/10 dark:bg-red-950/20 text-primary dark:text-primary-light text-[11px] font-bold rounded-lg border border-red-100 dark:border-red-900/30 hover:bg-red-100 transition-colors"
                                    >
                                        RATE & REVIEW
                                    </button>
                                )}
                    ...
                    <div className="text-center py-20">
                        <div className="bg-slate-100 dark:bg-gray-800 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4">
                            <Utensils className="w-8 h-8 text-slate-300 dark:text-text-secondary" />
                        </div>
                        <h3 className="text-lg font-bold text-gray-800 dark:text-white">No bookings yet</h3>
                        <p className="text-text-secondary dark:text-text-secondary text-sm mt-2">Book your favorite restaurant for a great dining experience!</p>
                        <Link to="/dining">
