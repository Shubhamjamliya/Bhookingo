...
export default function RestaurantOnboarding() {
    ...
    try {
      ...
      const applyLocationData = (locDetails) => {
        ...
        setStep1((prev) => {
          ...
          updated.location.roadName = locDetails.roadName || prev.location.roadName || "";
  ...
  const reverseGeocodePinnedLocation = useCallback((lat, lng) => {
    return new Promise((resolve) => {
  ...
  const syncPinnedLocation = useCallback(async (lat, lng) => {
  ...
  useEffect(() => {
    ...
    if (window.google?.maps) {
      if (!isMapsSdkReady) setIsMapsSdkReady(true)
      return undefined
    }

    let cancelled = false

    const loadMapsForPin = async () => {
      const apiKey = await getGoogleMapsApiKey()
      if (!apiKey || cancelled) return

      const waitForMapSdk = async () => {
        for (let i = 0; i < 60; i++) {
  ...
  useEffect(() => {
    ...
    if (!pinMapRef.current) {
      ...
      pinMarkerRef.current?.setDraggable(Boolean(isEditing))
    }

    return undefined
  }, [step, step1.location?.latitude, step1.location?.longitude, isEditing, isMapsSdkReady, syncPinnedLocation])
  ...
  const handleNext = async () => {
    ...
    try {
      if (step === 1) {
        ...
        if (hasExistingRestaurantProfile) {
          ...
          const updatePayload = {
            ...
            location: {
              ...
              roadName: step1.location?.roadName || "",
        ...
        formData.append("roadName", step1.location?.roadName || "")
  ...
  const renderStep1 = () => (
    <div className="space-y-6">
      ...
      <RestaurantAddressHighwaySection
        ...
        isProcessingLink={isProcessingLink || !isEditing}
        ...
        onLocationFieldChange={(field, value) =>
          setStep1({
            ...step1,
            location: { ...step1.location, [field]: value },
          })
        }
        onRoadNameChange={(value) => {
          ...
          setStep1({
            ...step1,
            location: { ...step1.location, roadName: value },
          })
        }}
        normalizePincode={normalizePincode}
        footerNote="Please ensure that this address is the same as mentioned on your FSSAI license."
      />
