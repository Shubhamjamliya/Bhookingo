...
const DishImage = ({ src, alt, className }) => (
  <div className={`relative overflow-hidden ${className}`}>
    {src ? (
      ...
      <div className="w-full h-full bg-gray-100 dark:bg-gray-800 flex items-center justify-center text-xs text-text-secondary">
        No image
      </div>
    )}
  </div>
...
function RestaurantDetailsContent() {
  ...
  useEffect(() => {
    const fetchRestaurant = async () => {
      ...
      try {
        const queryKey = ['restaurant-details', slug]
        const cachedRestaurant = queryClient.getQueryData(queryKey)
        ...
        let apiRestaurant = await queryClient.fetchQuery({
          queryKey,
          staleTime: 1000 * 60 * 5, // 5 min cache
          queryFn: async () => {
            let fetchedApiRestaurant = null;
            let response = null;
            ...
            try {
              ...
              if (response?.data?.success && response?.data?.data) {
                fetchedApiRestaurant = response.data.data
                debugLog('? Found restaurant in dining API:', fetchedApiRestaurant)
              } else {
            ...
            if (!fetchedApiRestaurant) {
              try {
                ...
                try {
                  ...
                  if (response?.data?.success && response?.data?.data) {
                    fetchedApiRestaurant = response.data.data
                    debugLog('? Found restaurant in restaurant API by slug/ID:', fetchedApiRestaurant)
                  }
                      ...
                      try {
                        ...
                        if (matchingRestaurant) {
                          ...
                          if (fullResponse.data && fullResponse.data.success && fullResponse.data.data) {
                            fetchedApiRestaurant = fullResponse.data.data
                            debugLog('? Found restaurant in restaurant API by name search:', fetchedApiRestaurant)
                            break
                          }
                        }
                      } catch (searchError) {
        ...
        if (apiRestaurant) {
          ...
          const menuQueryKey = ['restaurant-menu', normalizedLookupIds.join(',')]
          const cachedMenu = queryClient.getQueryData(menuQueryKey)
          ...
          if (normalizedLookupIds.length > 0) {
            ...
            try {
              ...
              let menuResponse = await queryClient.fetchQuery({
              ...
              debugLog('? Menu resolved from cache or network')
      ...
      <AnimatedPage>
        <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
          <div className="flex flex-col items-center gap-4 text-center">
            <AlertCircle className={`h-12 w-12 ${isNetworkError ? 'text-[var(--primary)]' : 'text-red-500'}`} />
            <div>
              <h2 className="text-lg font-semibold text-text-primary dark:text-white mb-1">
                {isNetworkError ? 'Connection Error' : isNotFoundError ? 'Restaurant not found' : 'Error'}
              </h2>
              <p className="text-sm text-text-secondary mb-4 max-w-md">{restaurantError}</p>
              {isNetworkError && (
      ...
      <AnimatedPage>
        <div className="min-h-screen bg-gray-50 flex items-center justify-center">
          <div className="flex flex-col items-center gap-4">
            ...
            <span className="text-sm text-text-secondary">Restaurant not found</span>
            <Button onClick={goBack} variant="outline">
              Go Back
            </Button>
          </div>
  ...
  return (
    <AnimatedPage
      id="scrollingelement"
      className={`min-h-screen bg-[#f8f9fa] dark:bg-[#0a0a0a] flex flex-col transition-all duration-300 ${shouldShowGrayscale ? 'grayscale opacity-75' : ''
        }`}
    >
      {isOutOfRange && (
      ...
      <div className="px-4 sm:px-6 md:px-8 lg:px-10 xl:px-12 pt-3 md:pt-4 lg:pt-5 pb-2 md:pb-3 bg-transparent">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          {/* Back Button */}
          <Button
            variant="outline"
            size="icon"
            className="rounded-full h-10 w-10 border-border dark:border-gray-800 shadow-sm bg-white dark:bg-[#1a1a1a]"
            onClick={goBack}
          >
          ...
          <div className="flex items-center gap-3">
            {!showSearch ? (
              <Button
                variant="outline"
                className="rounded-full h-10 px-4 border-border dark:border-gray-800 shadow-sm bg-surface dark:bg-[#1a1a1a] flex items-center gap-2 text-text-primary dark:text-white"
              ...
              <div className="flex items-center gap-2 flex-1 max-w-md">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-10 py-2 rounded-full border border-border dark:border-gray-800 shadow-sm bg-surface dark:bg-[#1a1a1a] text-sm dark:text-white focus:outline-none focus:ring-2 focus:ring-[var(--primary)] focus:border-transparent"
                    autoFocus
                    onBlur={() => {
                      if (!searchQuery) {
                        setShowSearch(false)
            ...
            <Button
              variant="outline"
              size="icon"
              className="rounded-full h-10 w-10 border-border dark:border-gray-800 shadow-sm bg-surface dark:bg-[#1a1a1a]"
              onClick={() => setShowMenuOptionsSheet(true)}
            >
              <MoreVertical className="h-5 w-5 text-text-primary dark:text-white" />
            </Button>
          </div>
        </div>
      </div>
      ...
      {/* Main Content Card */}
      <div className="bg-transparent relative z-10 min-h-[40vh]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 lg:px-10 xl:px-12 py-4 sm:py-5 md:py-6 lg:py-8 space-y-3 md:space-y-4 lg:space-y-5 pb-0">
          ...
          <div className="relative">
            <div className="relative rounded-[2rem] bg-black shadow-sm overflow-hidden flex flex-col min-h-[160px]">
              ...
              <div className="absolute inset-0 pointer-events-none z-0">
                {restaurant?.image && (
                  <>
                    ...
                    />
                    <div className="absolute inset-0 bg-gradient-to-r from-black/95 via-black/50 to-black/10 w-full" />
                  </>
                )}
              </div>
              ...
              <div className="relative z-10 flex flex-col h-full justify-between p-4 sm:p-5">
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1 pr-4">
                    <div className="flex items-center gap-1.5 mb-1">
                      <h1 className="text-[22px] font-black text-white leading-tight drop-shadow-sm">
                        {restaurant?.name || "Unknown Restaurant"}
                      </h1>
                      <div className="bg-[#257d3c] rounded-full p-0.5 flex-shrink-0">
                        <Check className="h-3 w-3 text-white" strokeWidth={3} />
                      </div>
                    </div>
                    
                    <div className="flex items-center gap-1.5 text-sm text-gray-300 font-medium mb-3">
                      <Utensils className="h-3.5 w-3.5 opacity-80" />
                      <span>{restaurant?.topCategory || restaurant?.cuisine || "Multi-cuisine"}</span>
                    </div>

                    <div className="flex items-center gap-1.5 text-xs text-gray-300 font-medium mb-3">
                      <MapPin className="h-3.5 w-3.5 opacity-80 flex-shrink-0" />
                      <span className="truncate">
                        {restaurant?.distance || "1.2 km"} &middot; {restaurant?.location || "Location"}
                      </span>
                    </div>

                    <div className="flex items-center gap-2">
                      <div className={`inline-flex items-center justify-center rounded-full px-2.5 py-1 text-[11px] font-bold border backdrop-blur-sm ${isRestaurantOffline ? "bg-red-500/20 text-red-300 border-red-500/30" : "bg-green-500/20 text-green-300 border-green-500/30"}`}>
                        <Clock className="h-3 w-3 mr-1" />
                        {isRestaurantOffline ? "Offline" : "Open now"}
                      </div>
                      <span className="text-[11px] font-medium text-gray-300 flex items-center gap-1">
                        Closes at 11:00 PM <Info className="h-3 w-3 opacity-80" />
                      </span>
                    </div>
                  </div>

                  {/* Top Right: Rating & Bookmark overlay */}
                  <div className="flex flex-col items-end gap-3 z-10 relative">
                    <Button
                      variant="outline"
                      size="icon"
                      className="rounded-full h-9 w-9 bg-black/40 backdrop-blur-md shadow-sm border border-white/20 hover:bg-black/60 transition-colors"
                      onClick={() => setShowShareModal(true)}
                    >
                      <Bookmark className="h-4 w-4 text-white" />
                    </Button>
                    <div className="flex flex-col items-center">
                      <div className="inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-sm font-bold text-white shadow-sm bg-[#257d3c]">
                        <Star className="h-3.5 w-3.5 fill-white" />
                        {Number(restaurant?.rating || 0) > 0 ? Number(restaurant.rating).toFixed(1) : "NEW"}
                      </div>
                      <span className="mt-1 text-[10px] font-bold text-gray-200 whitespace-nowrap bg-black/40 backdrop-blur-sm px-1.5 rounded">
                ...
                <div className="mt-6 pt-3 border-t border-white/20 flex items-center justify-between px-2">
                   <div className="flex flex-col items-center gap-1">
                     <div className="w-8 h-8 rounded-full bg-white/10 backdrop-blur-md flex items-center justify-center border border-white/20">
                       <AlertCircle className="h-4 w-4 text-white" />
                     </div>
                     <span className="text-[9px] font-medium text-gray-300">WiFi</span>
                   </div>
                   ...
                   <div className="flex flex-col items-center gap-1">
                     ...
                     <span className="text-[9px] font-medium text-gray-300">EV Charging</span>
                   </div>
                   <div className="w-[1px] h-6 bg-white/20" />
                   <div className="flex flex-col items-center gap-1">
                     <div className="w-8 h-8 rounded-full bg-white/10 backdrop-blur-md flex items-center justify-center border border-white/20">
                     ...
                     <span className="text-[9px] font-medium text-gray-300">Parking</span>
                   </div>
                   <div className="w-[1px] h-6 bg-white/20" />
                   <div className="flex flex-col items-center gap-1">
                     <div className="w-8 h-8 rounded-full bg-white/10 backdrop-blur-md flex items-center justify-center border border-white/20">
                     ...
                     <span className="text-[9px] font-medium text-gray-300">Washroom</span>
                   </div>
                   <div className="w-[1px] h-6 bg-white/20" />
                   <div className="flex flex-col items-center gap-1">
                     <div className="w-8 h-8 rounded-full bg-white/10 backdrop-blur-md flex items-center justify-center border border-white/20">
                     ...
                     <span className="text-[9px] font-medium text-gray-300">Hygiene</span>
                   </div>
                </div>
          ...
          {isRestaurantOffline && (
            <div className="rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-sm text-rose-700 shadow-sm mx-1">
              {restaurant?.name || "This restaurant"} is currently offline. Orders are unavailable right now.
            </div>
          )}

          {isOutOfRange && (
            <div className="rounded-xl border border-red-200 bg-red-50 dark:bg-red-950/20 dark:border-red-900/50 px-3 py-2 text-sm text-red-700 dark:text-red-400 font-medium shadow-sm mx-1">
          ...
          {restaurant?.menuSections && Array.isArray(restaurant.menuSections) && restaurant.menuSections.length > 0 && (
            <div className="py-4 -mx-4 px-4 overflow-x-auto scrollbar-hide bg-transparent">
              <div className="flex items-center gap-3 w-max">
                {/* Filters Row */}
                <Button
                  variant="outline"
                  size="sm"
                  className="flex items-center gap-1.5 whitespace-nowrap border-border rounded-full h-9 bg-white dark:bg-[#1a1a1a] shadow-sm relative"
                  onClick={() => setShowFilterSheet(true)}
                >
                  <SlidersHorizontal className="h-3.5 w-3.5" />
                  <span className="font-medium text-[13px]">Filters</span>
                  {activeFilterCount > 0 && (
                ...
                <Button
                  variant="outline"
                  size="sm"
                  className={`flex items-center gap-1.5 whitespace-nowrap border rounded-full h-9 bg-white dark:bg-[#1a1a1a] shadow-sm transition-all ${filters.vegNonVeg === "veg" ? "border-green-600 bg-green-50 text-green-700 font-bold dark:border-green-500 dark:bg-green-900/20 dark:text-green-400" : "border-border dark:border-gray-700 dark:text-white"
                    }`}
                  onClick={() =>
                    setFilters((prev) => ({
                      ...prev,
                ...
                >
                  ...
                  <span className="font-medium text-[13px]">Veg</span>
                </Button>
                <Button
                  variant="outline"
                  size="sm"
                  className={`flex items-center gap-1.5 whitespace-nowrap border rounded-full h-9 bg-white dark:bg-[#1a1a1a] shadow-sm transition-all ${filters.vegNonVeg === "non-veg" ? "border-primary bg-primary-light/10 text-primary dark:border-red-500 dark:bg-red-900/20 dark:text-primary-light" : "border-border dark:border-gray-700 dark:text-white"
                    }`}
                  onClick={() =>
                    setFilters((prev) => ({
                      ...prev,
                ...
                >
                  ...
                  <span className="font-medium text-[13px]">Non-veg</span>
                </Button>
                <div className="w-[1px] h-5 bg-gray-300 dark:bg-gray-700 mx-2" />
                <Button
                  variant="outline"
                  size="sm"
                  className="flex items-center gap-1.5 whitespace-nowrap border-border rounded-full h-9 bg-white dark:bg-[#1a1a1a] shadow-sm text-gray-700 dark:text-gray-300 ml-auto"
                  onClick={() => setShowFilterSheet(true)}
                >
                  <span className="font-medium text-[13px]">Sort by</span>
                  <ChevronDown className="h-3.5 w-3.5" />
                </Button>
              </div>

              {menuCategories.length > 0 && (
                <div className="flex items-center gap-3 w-max mt-4">
                  <button
                    type="button"
                    onClick={() => setSelectedMenuCategory("all")}
                    className={`flex items-center gap-2 whitespace-nowrap rounded-full border px-4 py-2 text-[13px] font-semibold transition-all shadow-sm ${
                  ...
                  {menuCategories.map((category) => (
                    <button
                      ...
                      className={`flex items-center gap-2 whitespace-nowrap rounded-full border px-3 py-1.5 text-[13px] font-semibold transition-all shadow-sm ${
        ...
        {restaurant && (
          <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 lg:px-10 xl:px-12 py-6 sm:py-8 md:py-10 lg:py-12 space-y-6 md:space-y-8 lg:space-y-10">
            {filteredSections.length === 0 && hasActiveMenuFilters && (
              <div className="rounded-2xl border border-dashed border-border dark:border-gray-700 bg-surface dark:bg-[#1a1a1a] px-5 py-8 text-center">
                <p className="text-sm md:text-base font-medium text-gray-700 dark:text-gray-300">
                  No dishes match the selected filters.
                </p>
                <p className="text-xs md:text-sm text-text-secondary dark:text-text-secondary mt-2">
              ...
              return (
                <div key={sectionIndex} id={sectionId} className="space-y-1 scroll-mt-20">
                  <div className="flex items-center justify-between">
                    ...
                    >
                      <ChevronDown
                        className={`h-5 w-5 text-text-secondary dark:text-text-secondary transition-transform duration-200 ${isExpanded ? '' : '-rotate-90'
                          }`}
                      />
                    </button>
                  </div>
                      ...
                      {sectionItems.map((item) => {
                        ...
                        return (
                          <div
                            ...
                            className={`bg-white dark:bg-[#1a1a1a] rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm p-3 mb-4 mx-2 sm:mx-0 relative cursor-pointer transition-all duration-300 ${highlightedDishId === item.id && isRecommended ? "ring-2 ring-red-500/20" : ""}`}
                            onClick={() => handleItemClick(item)}
                          >
                            {/* New Layout - Image on Left, Details on Right */}
                              ...
                              <div className="relative w-[110px] h-[110px] flex-shrink-0">
                                {item.image ? (
                                  <OptimizedImage
                                    ...
                                    alt={item.name}
                                    priority={isPriority}
                                    className="w-full h-full object-cover rounded-[1rem] shadow-sm border border-gray-100 dark:border-gray-800"
                                    onError={(e) => {
                                  ...
                                  <div className="w-full h-full bg-gray-100 dark:bg-gray-800 rounded-[1rem] border border-gray-200 dark:border-gray-700 flex items-center justify-center">
                                    <span className="text-xs text-text-secondary">No image</span>
                                  </div>
                                )}
                              </div>

                              {/* Right Side - Details and Add Button */}
                              <div className="flex-1 min-w-0 flex flex-col relative pb-8">
                                {/* Top row: Title and Rating */}
                                <div className="flex justify-between items-start gap-2 mb-1">
                                  <div className="flex items-start gap-1.5">
                                    <div className="mt-[4px] shrink-0">
                                      {isVeg ? (
                                        <div className="w-3.5 h-3.5 border-[1.5px] border-green-600 flex items-center justify-center rounded-[2px] p-[1.5px]">
                                          <div className="w-full h-full bg-green-600 rounded-full"></div>
                                        </div>
                                      ) : (
                                        <div className="w-3.5 h-3.5 border-[1.5px] border-red-500 flex items-center justify-center rounded-[2px] p-[1.5px]">
                                          <div className="w-full h-full bg-red-500 rounded-full"></div>
                                        </div>
                                      )}
                                    </div>
                                    <h3 className="font-bold text-gray-900 dark:text-white text-[16px] leading-tight tracking-tight">
                                      {item.name}
                                    </h3>
                                  </div>
                                  {/* Rating Badge */}
                                  <div className="flex flex-col items-end shrink-0 mt-[2px]">
                                    <div className="flex items-center gap-0.5 text-[11px] font-bold text-gray-700 dark:text-gray-300">
                                ...
                                <div className="flex items-center gap-3 mb-2">
                                  <p className="font-bold text-gray-900 dark:text-white text-[16px]">{getFoodPriceLabel(item)}</p>
                                  {item.preparationTime && String(item.preparationTime).trim() && (
                                    <div className="flex items-center gap-1 text-[11px] font-medium text-gray-500 bg-gray-50 dark:bg-gray-800 px-1.5 py-0.5 rounded-md">
                                      <Clock size={10} />
                                ...
                                {/* Bookmark & Share Below Description */}
                                ...
                                <div className="absolute -bottom-2 -right-1">
                                  {quantity > 0 ? (
                                    <div
                                      className={`bg-[#ff5200] text-white font-bold px-3 py-1.5 rounded-full shadow-md flex items-center gap-2 ${shouldShowGrayscale || isOutOfRange
                                    ...
                                    <button
                                      ...
                                      disabled={shouldShowGrayscale || isOutOfRange}
                                      className={`bg-[#ff5200] text-white font-bold px-5 py-1.5 rounded-full shadow-md flex items-center gap-1 transition-all ${shouldShowGrayscale || isOutOfRange
                                        ? 'bg-gray-300 cursor-not-allowed opacity-50'
                                        : 'hover:bg-[#e64a00] hover:scale-105 active:scale-95'
                                        }`}
                                    >
                                      {isOutOfRange ? "N/A" : <>ADD <Plus size={14} strokeWidth={3} /></>}
                                    </button>
                                  )}
                                </div>
                        ...
                        return (
                          <div key={subIndex} className="space-y-4">
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              >
                                <ChevronDown
                                  className={`h-4 w-4 text-text-secondary dark:text-text-secondary transition-transform duration-200 ${isSubsectionExpanded ? '' : '-rotate-90'
                                    }`}
                                />
                              </button>
                            </div>
                                ...
                                {subsectionItems.map((item) => {
                                  ...
                                  return (
                                    <div
                                      ...
                                      className={`bg-white dark:bg-[#1a1a1a] rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm p-3 mb-4 mx-2 sm:mx-0 relative cursor-pointer transition-all duration-300 ${highlightedDishId === item.id ? "ring-2 ring-red-500/20" : ""}`}
                                      onClick={() => handleItemClick(item)}
                                    >
                                      {/* New Layout - Image on Left, Details on Right */}
                                      <div className="flex gap-4">
        ...
        <div className="px-4 py-4 mt-2 mb-8 border-t border-dashed border-border dark:border-gray-800 bg-gray-50/30 dark:bg-white/5 mx-4 rounded-xl">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <p className="text-[10px] text-text-secondary dark:text-text-secondary uppercase tracking-widest font-bold mb-1">
                License No.
              </p>
              <p className="text-sm font-semibold text-text-secondary dark:text-gray-300 font-mono tracking-wide">
                {restaurant?.onboarding?.step3?.fssai?.registrationNumber}
              </p>
            </div>
          </div>
      ...
      {typeof window !== "undefined" && !showFilterSheet && !showMenuSheet && !showMenuOptionsSheet && restaurant && filteredSections.length > 0 &&
        createPortal(
          ...
          >
            <Button
              ref={menuButtonRef}
              className="bg-[#1a1a1a] hover:bg-black text-white flex items-center gap-2 shadow-[0_8px_30px_rgba(0,0,0,0.3)] border border-white/20 px-5 py-3 h-auto rounded-full font-bold active:scale-95 group transition-colors"
              size="default"
              onClick={() => setShowMenuSheet(true)}
            >
              <Utensils className="h-4 w-4 text-white group-hover:rotate-12 transition-transform" />
              <span className="tracking-widest text-[11px] uppercase font-extrabold antialiased">Menu</span>
            </Button>
          </motion.div>,
          document.body
        )
                        ...
                        >
                          <div className="flex items-center gap-3 min-w-0">
                            {category.image ? (
                              ...
                              <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-gray-100 text-sm font-bold uppercase text-text-secondary">
                                {category.name?.charAt(0) || "C"}
                              </span>
                            )}
                            <span className="text-base font-medium text-text-primary dark:text-white truncate">
                              {category.name}
                            </span>
                          </div>
                          <div className="flex items-center gap-2">
            ...
            {showFilterSheet && (
              <>
                ...
                >
                  ...
                  <div className="flex items-center justify-between px-4 pt-3 pb-2 border-b border-border dark:border-gray-800">
                    <h2 className="text-lg font-semibold text-text-primary dark:text-white">Filters and Sorting</h2>
                    <button
                      onClick={() => setShowFilterSheet(false)}
                      className="p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-full transition-colors"
                    >
                      <X className="h-5 w-5 text-text-secondary dark:text-text-secondary" />
                    </button>
                  </div>
                  ...
                  <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4">
                    ...
                    <div className="space-y-2">
                      <h3 className="text-sm font-semibold text-text-primary dark:text-white">Sort by:</h3>
                    ...
                    <div className="space-y-2">
                      <h3 className="text-sm font-semibold text-text-primary dark:text-white">Veg/Non-veg preference:</h3>
                    ...
                    {/* Top picks */}
                    <div className="space-y-2">
                      <h3 className="text-sm font-semibold text-text-primary dark:text-white">Top picks:</h3>
                      <button
                        onClick={() =>
                          setFilters((prev) => ({
                            ...prev,
                    ...
                    {/* Dietary preference */}
                    <div className="space-y-2">
                      <h3 className="text-sm font-semibold text-text-primary dark:text-white">Dietary preference:</h3>
                      <button
                        onClick={() =>
                          setFilters((prev) => ({
                            ...prev,
                  ...
                  <div className="border-t border-border dark:border-gray-800 px-4 py-3 flex items-center justify-between bg-white dark:bg-[#1a1a1a]">
                    <button
                      ...
                      className="text-primary dark:text-primary-light font-medium text-sm hover:text-primary-dark dark:hover:text-red-500"
                    >
                      Clear All
                    </button>
                    <Button
                      className="bg-[var(--primary)] hover:bg-primary-dark text-white px-6 py-2.5 rounded-lg font-bold"
                      onClick={() => setShowFilterSheet(false)}
                    >
          ...
          <AnimatePresence>
            {showLocationSheet && (
              <>
                ...
                {/* Bottom Sheet */}
                <motion.div
                  className="fixed left-0 right-0 bottom-0 md:left-1/2 md:right-auto md:-translate-x-1/2 md:bottom-auto md:top-1/2 md:-translate-y-1/2 z-[10000] bg-surface dark:bg-[#1a1a1a] rounded-t-3xl md:rounded-3xl shadow-2xl h-[75vh] md:h-auto md:max-h-[90vh] md:max-w-xl w-full md:w-auto flex flex-col"
                  initial={{ y: "100%" }}
                  animate={{ y: 0 }}
                  exit={{ y: "100%" }}
                  transition={{ duration: 0.2, type: "spring", damping: 30, stiffness: 400 }}
                ...
                >
                  {/* Header */}
                  <div className="px-4 pt-4 pb-3 border-b border-border dark:border-gray-800">
                    <div className="flex items-center gap-2">
                      <div className="w-8 h-8 bg-primary dark:bg-red-500 rounded-lg flex items-center justify-center">
                        <span className="text-white font-bold text-base">{(restaurant.name || "R").charAt(0).toUpperCase()}</span>
                      </div>
                      <h2 className="text-lg font-bold text-text-primary dark:text-white">{restaurant?.name || "Unknown Restaurant"}</h2>
                    </div>
                  </div>
                      ...
                      <div className="space-y-2">
                        {restaurant.outlets.map((outlet) => (
                          <div
                            ...
                            className="p-3 rounded-lg border border-border dark:border-gray-700 bg-surface dark:bg-[#2a2a2a]"
                          >
                            {outlet?.isNearest && (
                              <div className="flex items-center gap-1.5 mb-2 px-2 py-1 bg-background dark:bg-[var(--primary)]/20 rounded-md">
                                <Zap className="h-3.5 w-3.5 text-[var(--primary)] dark:text-[var(--primary)] fill-[var(--primary)] dark:fill-[var(--primary)]" />
                                <span className="text-xs font-semibold text-[var(--primary)] dark:text-[var(--primary)]">
                  ...
                  {restaurant?.outlets && Array.isArray(restaurant.outlets) && restaurant.outlets.length > 5 && (
                    <div className="border-t border-border dark:border-gray-800 px-4 py-3 bg-surface dark:bg-[#1a1a1a]">
                      <button className="flex items-center justify-center gap-2 text-primary dark:text-primary-light font-medium text-sm w-full">
          ...
          <AnimatePresence>
            {showManageCollections && (
              <>
                ...
                <motion.div
                  className="fixed left-0 right-0 bottom-0 md:left-1/2 md:right-auto md:-translate-x-1/2 md:bottom-auto md:top-1/2 md:-translate-y-1/2 z-[10000] bg-surface dark:bg-[#1a1a1a] rounded-t-3xl md:rounded-3xl shadow-2xl md:max-w-lg w-full md:w-auto"
                  initial={{ y: "100%" }}
                  animate={{ y: 0 }}
                  exit={{ y: "100%" }}
                  transition={{ duration: 0.2, type: "spring", damping: 30, stiffness: 400 }}
                >
                  {/* Header */}
                  <div className="flex items-center justify-between px-4 pt-6 pb-4 border-b border-border dark:border-gray-800">
                    <h2 className="text-lg font-bold text-text-primary dark:text-white">Manage Collections</h2>
                    <button
                      onClick={() => setShowManageCollections(false)}
                  ...
                  <div className="px-4 py-4 space-y-2">
                    ...
                    >
                      <div className="h-12 w-12 rounded-lg bg-pink-100 dark:bg-pink-900/30 flex items-center justify-center flex-shrink-0">
                        <Bookmark className="h-6 w-6 text-red-500 dark:text-primary-light fill-red-500 dark:fill-red-400" />
                      </div>
                      <div className="flex-1 text-left">
                        <div className="flex items-center justify-between">
                          <span className="text-base font-medium text-text-primary dark:text-white">Bookmarks</span>
                          {selectedItem && (
                            <Checkbox
                    ...
                    >
                      <div className="h-12 w-12 rounded-lg bg-pink-100 dark:bg-pink-900/30 flex items-center justify-center flex-shrink-0">
                        <Plus className="h-6 w-6 text-red-500 dark:text-primary-light" />
                      </div>
                      <div className="flex-1 text-left">
                  ...
                  {/* Done Button */}
                  <div className="border-t border-border dark:border-gray-800 px-4 py-4">
                    <Button
                      className="w-full bg-[var(--primary)] hover:bg-primary-dark text-white py-3 rounded-lg font-bold"
                      onClick={() => {
                        setShowManageCollections(false)
                      }}
                    >
                    ...
                    <div className="absolute bottom-4 right-4 flex items-center gap-3">
                      <button
                        ...
                        className={`h-10 w-10 rounded-full border flex items-center justify-center transition-all duration-300 ${isDishFavorite(selectedItem.id, restaurant?.restaurantId || restaurant?._id || restaurant?.id)
                          ? "border-red-500 dark:border-red-400 bg-primary-light/10 dark:bg-red-900/30 text-red-500 dark:text-primary-light"
                          : "border-white dark:border-gray-800 bg-white/90 dark:bg-[#1a1a1a]/90 text-text-secondary dark:text-gray-300 hover:bg-white dark:hover:bg-[#2a2a2a]"
                          }`}
                      >
                        <Bookmark
                      ...
                      <button className="h-10 w-10 rounded-full border border-white dark:border-gray-800 bg-white/90 dark:bg-[#1a1a1a]/90 text-text-secondary dark:text-gray-300 hover:bg-white dark:hover:bg-[#2a2a2a] flex items-center justify-center transition-colors">
                  ...
                  <div className="flex-1 overflow-y-auto px-4 py-4">
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-2 flex-1">
                        <div className={`h-5 w-5 rounded border-2 ${selectedItem.foodType === "Veg" ? "border-green-600 bg-green-50" : "border-primary bg-primary-light/10"} dark:border-gray-600 dark:bg-gray-900/30 flex items-center justify-center flex-shrink-0`}>
                          <div className={`h-2.5 w-2.5 rounded-full ${selectedItem.foodType === "Veg" ? "bg-green-600" : "bg-primary"}`} />
                        </div>
                    ...
                    {/* Description */}
                    <p className="text-sm text-text-secondary dark:text-text-secondary mb-4 leading-relaxed">
                    ...
                    {hasFoodVariants(selectedItem) && (
                      <div className="mb-4">
                        <p className="text-sm font-semibold text-text-primary dark:text-white mb-2">Choose a variant</p>
                            ...
                            <button
                              ...
                              className={`rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
                                String(selectedVariantId || "") === String(variant.id)
                                  ? "border-red-500 bg-primary-light/10 text-primary dark:border-red-400 dark:bg-red-900/30 dark:text-red-200"
                                  : "border-border bg-white text-gray-700 dark:border-gray-700 dark:bg-[#2a2a2a] dark:text-gray-300"
                              }`}
                            >
          ...
          <AnimatePresence>
            {showScheduleSheet && (
              <>
                ...
                <motion.div
                  className="fixed left-0 right-0 bottom-0 md:left-1/2 md:right-auto md:-translate-x-1/2 md:bottom-auto md:top-1/2 md:-translate-y-1/2 z-[10000] bg-surface dark:bg-[#1a1a1a] rounded-t-3xl md:rounded-3xl shadow-2xl max-h-[60vh] md:max-h-[90vh] md:max-w-lg w-full md:w-auto flex flex-col"
                  initial={{ y: "100%" }}
                  animate={{ y: 0 }}
                  exit={{ y: "100%" }}
                ...
                >
                  ...
                  <div className="flex-1 overflow-y-auto px-4 pt-10 pb-4">
                    {/* Title */}
                    <h2 className="text-lg font-bold text-text-primary dark:text-white mb-4 text-center">
                    </h2>
                      ...
                      {(() => {
                        ...
                        return dates.map((item, index) => {
                          ...
                          return (
                            ...
                            >
                              <span className={`text-sm font-medium ${isSelected ? 'text-text-primary dark:text-white' : 'text-text-secondary dark:text-text-secondary'}`}>
                        ...
                        return (
                          <button
                            ...
                            className={`w-full text-left px-4 py-2.5 rounded-lg transition-all ${isSelected
                              ? "bg-gray-100 dark:bg-gray-800 text-text-primary dark:text-white border border-border dark:border-gray-600"
                              : "bg-surface dark:bg-[#2a2a2a] text-text-secondary dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 border border-transparent"
                              }`}
                          >
                  ...
                  <div className="px-4 pb-4 pt-2 border-t border-border">
                    <Button
                      className="w-full bg-red-500 hover:bg-primary text-white py-3 rounded-lg font-semibold"
                      onClick={() => {
          ...
          <AnimatePresence>
            {showMenuOptionsSheet && (
              <>
                ...
                >
                  {/* Header */}
                  <div className="px-4 pt-6 pb-4 border-b border-border dark:border-gray-800">
                    <h2 className="text-lg font-bold text-text-primary dark:text-white">
                      {restaurant?.name || "Unknown Restaurant"}
                    </h2>
                  </div>
                  ...
                  <div className="flex-1 overflow-y-auto px-4 py-4">
                    ...
                    <div className="space-y-1">
                      ...
                      >
                        ...
                        <span className="text-base text-text-primary dark:text-white">
                      ...
                      >
                        ...
                        <span className="text-base text-text-primary dark:text-white">Share this restaurant</span>
                      </button>
                    ...
                    {restaurant?.onboarding?.step3?.fssai?.registrationNumber && (
                      <div className="mt-4 px-2 pt-4 border-t border-border dark:border-gray-800 flex items-center gap-3 opacity-80 mb-2">
                        ...
                        <div className="flex-1">
                          <p className="text-[10px] text-text-secondary dark:text-text-secondary uppercase tracking-wide font-medium">
                            Lic. No.
                          </p>
