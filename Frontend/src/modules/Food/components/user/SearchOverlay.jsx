...
export default function SearchOverlay({ isOpen, onClose, searchValue, onSearchChange, autoStartVoice = false }) {
  ...
  return (
    ...
    >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <form onSubmit={handleSearchSubmit} className="flex items-center gap-4">
            <div className="flex-1 relative">
              <Search className="absolute left-5 top-1/2 transform -translate-y-1/2 h-5 w-5 text-[var(--primary)] dark:text-[var(--primary-light)] z-10" strokeWidth={2.5} />
              <Input
                ref={inputRef}
                value={searchValue}
                onChange={(e) => onSearchChange(e.target.value)}
                placeholder="Search dishes or restaurants"
                className="pl-14 pr-16 h-13 w-full bg-surface dark:bg-[#1a1a1a] border-border dark:border-gray-800 focus:ring-2 focus:ring-[var(--primary)]/20 focus:border-[var(--primary)] dark:focus:border-[var(--primary)] rounded-2xl text-base dark:text-white placeholder:text-text-secondary dark:placeholder:text-text-secondary shadow-sm transition-all duration-200"
              />
              <div className="absolute right-4 top-1/2 transform -translate-y-1/2 flex items-center gap-3">
                ...
                {isListening ? (
                  <button
                    ...
                    className="p-2.5 rounded-xl bg-primary-light/10 dark:bg-red-900/20 text-red-500 animate-pulse shadow-sm"
                  >
                  ...
                  <button
                    ...
                    className="p-2.5 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800 text-[var(--primary)] dark:text-[var(--primary-light)] transition-all active:scale-90"
                  >
                    <Mic className="h-5 w-5" />
                  </button>
                )}
      ...
      <div className="flex-1 overflow-y-auto max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-6 scrollbar-hide bg-surface dark:bg-[#0a0a0a]">
        {/* Suggestions Row */}
        <div
          className="mb-6"
          style={{
        ...
        >
          ...
          <div className="flex gap-2 sm:gap-3 flex-wrap">
            {recentSuggestions.slice(0, 8).map((suggestion, index) => (
              ...
              >
                <Clock className="h-3 w-3 sm:h-4 sm:w-4 text-[var(--primary)] flex-shrink-0" />
        ...
        >
          <h3 className="text-lg sm:text-xl font-bold text-text-primary dark:text-white mb-4 sm:mb-6">
          ...
          {filteredFoods.length > 0 ? (
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-3 sm:gap-4 md:gap-5 lg:gap-6">
              {filteredFoods.map((food, index) => (
                ...
                >
                  <div className="relative w-full aspect-square rounded-full overflow-hidden transition-all duration-200 shadow-md group-hover:shadow-lg bg-surface dark:bg-[#1a1a1a] p-1 sm:p-1.5">
                    {food.image ? (
                      <img
                        src={food.image}
                        alt={food.name}
                  ...
                  <div className="px-1 sm:px-2 text-center">
                    <span className="text-xs sm:text-sm font-semibold text-gray-800 dark:text-gray-200 group-hover:text-[var(--primary)] dark:group-hover:text-orange-400 transition-colors line-clamp-2">
                      {food.name}
                    </span>
                  </div>
                </div>
            ...
            <div className="text-center py-12 sm:py-16">
              {loadingFoods ? (
                <>
                  <Loader2 className="h-12 w-12 sm:h-16 sm:w-16 text-gray-300 dark:text-text-secondary mx-auto mb-4 animate-spin" />
                  <p className="text-text-secondary dark:text-text-secondary text-base sm:text-lg font-semibold">Loading dishes from database...</p>
                </>
              ) : (
                <>
                  <Search className="h-12 w-12 sm:h-16 sm:w-16 text-gray-300 dark:text-text-secondary mx-auto mb-4" />
                  <p className="text-text-secondary dark:text-text-secondary text-base sm:text-lg font-semibold">
                    {searchValue.trim() ? `No results found for "${searchValue}"` : "No dishes found in database"}
                  </p>
                  <p className="text-sm sm:text-base text-text-secondary dark:text-text-secondary mt-2">
