...
export default function Support() {
  ...
  const TicketList = () => (
    <Card className="bg-surface dark:bg-[#1a1a1a] rounded-xl shadow-sm border border-border dark:border-gray-800">
      <CardContent className="p-4">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-base font-semibold text-text-primary dark:text-white">My Tickets</h3>
          <span className="text-xs font-medium px-2 py-1 rounded bg-slate-100 dark:bg-slate-800 text-text-secondary dark:text-slate-300">
            {tickets.length}
          </span>
        </div>
        ...
        {loadingTickets ? (
          ...
          <div className="space-y-2">
            {tickets.map((t) => (
              <div key={t._id || t.id} className="border border-border dark:border-slate-700 rounded-lg p-3 bg-surface dark:bg-[#171717]">
                ...
                {t.adminResponse ? (
                  <p className="text-xs text-text-secondary dark:text-slate-300 mt-2">Reply: {t.adminResponse}</p>
                ) : null}
              </div>
            ))}
          </div>
  ...
  return (
    <AnimatedPage className="min-h-screen bg-background dark:bg-[#0a0a0a]">
      <div className="max-w-md md:max-w-2xl lg:max-w-4xl xl:max-w-5xl mx-auto px-4 sm:px-6 md:px-8 lg:px-10 xl:px-12 py-4 sm:py-6 md:py-8 pb-20">
        ...
        <Card className="bg-surface dark:bg-[#1a1a1a] rounded-xl shadow-sm border border-border dark:border-gray-800 mb-3">
          <CardContent className="p-4">
            <h1 className="text-xl font-bold text-text-primary dark:text-white">Help & Support</h1>
            <p className="text-sm text-text-secondary mt-1">Raise a support ticket and track updates in one place.</p>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4 space-y-4">
            {step === "pick" && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <button onClick={() => handlePick("order")} className="w-full border border-border dark:border-slate-700 rounded-xl p-4 text-left hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                  <div className="flex items-center justify-between">
                    <ShoppingBag className="h-5 w-5 text-slate-700 dark:text-slate-200" />
                    <ChevronRight className="h-4 w-4 text-text-secondary" />
                  </div>
                  <p className="mt-3 font-semibold text-text-primary dark:text-white">Order Issue</p>
                </button>

                <button onClick={() => handlePick("restaurant")} className="w-full border border-border dark:border-slate-700 rounded-xl p-4 text-left hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                  <div className="flex items-center justify-between">
                    <Building2 className="h-5 w-5 text-slate-700 dark:text-slate-200" />
                    <ChevronRight className="h-4 w-4 text-text-secondary" />
                  </div>
                  <p className="mt-3 font-semibold text-text-primary dark:text-white">Restaurant Issue</p>
                  <p className="text-xs text-text-secondary mt-1">Service, listing info, behavior report</p>
                </button>

                <button onClick={() => handlePick("other")} className="w-full border border-border dark:border-slate-700 rounded-xl p-4 text-left hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                  <div className="flex items-center justify-between">
                    <HelpCircle className="h-5 w-5 text-slate-700 dark:text-slate-200" />
                    <ChevronRight className="h-4 w-4 text-text-secondary" />
                  </div>
                  <p className="mt-3 font-semibold text-text-primary dark:text-white">Other Issue</p>
                  <p className="text-xs text-text-secondary mt-1">Account, app, payment or general query</p>
                </button>
              </div>
            )}
            ...
            {step === "choose_order" && (
              <div className="space-y-3">
                <h3 className="font-semibold text-text-primary dark:text-white">Select an order</h3>
                {orders.length > 0 ? (
                  <div className="space-y-2">
                    <Input
                    ...
                    {filteredOrders.length === 0 ? <p className="text-sm text-text-secondary">No matching orders found</p> : null}
                  </div>
                ) : (
                  <p className="text-sm text-text-secondary">No recent orders found</p>
                )}
            ...
            {step === "order_issue" && selectedOrder && (
              <div className="space-y-3">
                <h3 className="font-semibold text-text-primary dark:text-white">Issue type</h3>
            ...
            {step === "choose_restaurant" && (
              <div className="space-y-3">
                <h3 className="font-semibold text-text-primary dark:text-white">Select a restaurant</h3>
                {restaurants.length > 0 ? (
                  <div className="space-y-2">
                    ...
                    {filteredRestaurants.length === 0 ? <p className="text-sm text-text-secondary">No matching restaurants found</p> : null}
                  </div>
                ) : (
                  <p className="text-sm text-text-secondary">No restaurants found</p>
                )}
                <Button variant="outline" onClick={() => setStep("pick")}>Back</Button>
              </div>
            )}
